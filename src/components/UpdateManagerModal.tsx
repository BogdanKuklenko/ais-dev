  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      <div className="bg-white dark:bg-[#15171C] border border-[#E5E5E0] dark:border-[#26282E] rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-[#111215] dark:text-white transition-colors">
        ...
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-[#E5E5E0] dark:border-[#26282E] flex items-center justify-between gap-3 bg-[#FBFBFA] dark:bg-[#181A20]">
          <div className="flex items-center gap-3.5">
            <div className="w-11 h-11 rounded-xl bg-[#111215] dark:bg-white text-white dark:text-[#111215] flex items-center justify-center shadow-xs shrink-0">
              <RefreshCw className="w-6 h-6 text-[#E63B00]" />
            </div>
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <span className="px-2.5 py-0.5 rounded-lg bg-[#F0F0EB] dark:bg-[#252830] text-[#5E6472] dark:text-[#8E95A5] font-mono text-xs sm:text-sm font-bold border border-[#E0E0D9] dark:border-[#2D3039]">
                  v{CURRENT_APP_VERSION}
                </span>
              </div>
              <p className="text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5] mt-1">
        ...
        {statusMessage && (
          <div className={`px-5 py-3 text-sm font-semibold flex items-center justify-between border-b ${
            ...
            <button onClick={() => setStatusMessage(null)} className="opacity-70 hover:opacity-100 p-1">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        {/* 2 Tabs Only */}
        <div className="px-4 sm:px-6 pt-3.5 border-b border-[#E5E5E0] dark:border-[#26282E] flex gap-3 bg-white dark:bg-[#15171C]">
          <button
            onClick={() => setActiveTab('network')}
            id="tab-btn-update-network"
            className={`pb-3.5 px-4 text-sm sm:text-base font-bold flex items-center gap-2 border-b-2 transition ${
              activeTab === 'network'
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {activeTab === 'network' && (
            <div className="space-y-5 animate-fadeIn">
              ...
              <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-[#111215] to-[#1A1C22] text-white border border-[#2D3039] shadow-xs">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <div className="text-xs sm:text-sm text-[#A0A6B5] font-mono mt-1 flex flex-wrap items-center gap-2">
                        <span>Сервер: <strong className="text-white">{serverUrl.replace(/\/update-manifest\.json.*$/, '')}</strong></span>
                        {networkLatency !== null && (
                          <>
                            <span>•</span>
                            <span className="text-emerald-400 font-bold">Пинг: {networkLatency} мс</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-[#E63B00] hover:bg-[#CC3400] text-white text-xs sm:text-sm font-bold flex items-center gap-2 shadow-xs transition disabled:opacity-50"
                    >
              ...
              <div className="p-4 rounded-xl bg-[#F8F8F5] dark:bg-[#1A1C22] border border-[#E5E5E0] dark:border-[#26282E] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs sm:text-sm">
                <div className="flex items-center gap-2.5 overflow-hidden">
                  <Server className="w-4 h-4 text-[#5E6472] dark:text-[#8E95A5] shrink-0" />
                  <span className="text-[#5E6472] dark:text-[#8E95A5] shrink-0">Адрес манифеста сервера:</span>
                  <strong className="font-mono text-[#111215] dark:text-white truncate">{serverUrl}</strong>
                </div>

                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={() => setIsEditingServerUrl(!isEditingServerUrl)}
                    className="px-3 py-1.5 rounded-lg bg-[#EFEFEA] hover:bg-[#E4E4DE] dark:bg-[#252830] dark:hover:bg-[#2F323D] text-xs font-bold text-[#5E6472] dark:text-[#8E95A5] transition"
                  >
              ...
              {/* Edit URL Form */}
              {isEditingServerUrl && (
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 space-y-2.5 text-sm">
                  <label className="font-bold text-slate-700 dark:text-slate-300 block text-xs sm:text-sm">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2 rounded-xl bg-white dark:bg-[#15171C] border border-[#CBD5E1] dark:border-[#334155] font-mono text-xs sm:text-sm text-[#111215] dark:text-white"
                    />
                    <button
                      onClick={() => {
                      ...
                      className="px-4 py-2 rounded-xl bg-[#111215] dark:bg-white text-white dark:text-[#111215] font-bold text-xs sm:text-sm"
                    >
                      Сохранить
                    </button>
                  </div>
              ...
              {/* Network Check Error */}
              {networkCheckError && (
                <div className="p-4 sm:p-5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/40 text-red-800 dark:text-red-300 text-xs sm:text-sm flex items-start gap-3">
              ...
              {networkManifest && (
                <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-[#FBFBFA] to-[#F1F1EB] dark:from-[#1A1C22] dark:to-[#14161B] border-2 border-emerald-500/40 dark:border-emerald-500/30 shadow-md space-y-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="flex items-center gap-3.5">
                      <div className="w-14 h-14 rounded-2xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-black text-xl font-mono">
                        v{networkManifest.latestVersion}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="px-2.5 py-0.5 rounded-md text-xs font-black uppercase tracking-wider bg-emerald-600 text-white">
                            Официальный релиз завода
                          </span>
                          <span className="text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5] font-mono">
                            Дата: {networkManifest.releaseDate} • {networkManifest.serverProtocol || 'HTTPS'}
                          </span>
                        </div>
                        <h3 className="text-base sm:text-lg font-black text-[#111215] dark:text-white mt-1">
                    ...
                    <button
                      ...
                      className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-black flex items-center gap-2 shadow transition disabled:opacity-50"
                    >
                  ...
                  {/* Changelog list */}
                  <div className="p-4 rounded-xl bg-white dark:bg-[#121316] border border-[#E5E5E0] dark:border-[#26282E] space-y-2.5">
                    <span className="text-xs sm:text-sm font-bold text-[#717684] dark:text-[#8E95A5] uppercase block">
                      Состав пакета и изменения:
                    </span>
                    <ul className="space-y-2 text-xs sm:text-sm text-[#111215] dark:text-slate-200">
                  ...
                  {isInstallingNetwork && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs sm:text-sm font-mono font-bold text-emerald-600">
              ...
              {!networkManifest && !networkCheckError && !isCheckingNetwork && (
                <div className="p-6 sm:p-8 text-center bg-[#F8F8F5] dark:bg-[#1A1C22] rounded-2xl border border-[#E5E5E0] dark:border-[#26282E] space-y-3">
                  <ShieldCheck className="w-10 h-10 mx-auto text-emerald-600" />
                  <h4 className="font-black text-base text-[#111215] dark:text-white">
                    Прямая синхронизация с облачным сервером по SSL
                  </h4>
                  <p className="text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5] max-w-lg mx-auto leading-relaxed">
          ...
          {activeTab === 'file' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-6 sm:p-10 text-center cursor-pointer transition flex flex-col items-center justify-center gap-3.5 ${
                  dragActive 
              ...
              >
                <input
                  ...
                  className="hidden"
                />
                <div className="w-14 h-14 rounded-2xl bg-[#EFEFEA] dark:bg-[#252830] text-[#E63B00] flex items-center justify-center shadow-inner">
                  <Upload className="w-7 h-7" />
                </div>
                <div>
                  <h4 className="font-black text-base sm:text-lg text-[#111215] dark:text-white">
                    Перетащите файл патча (.alex-patch, .patch или .json) сюда
                  </h4>
                  <p className="text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5] mt-1.5">
              ...
              {/* Error box */}
              {patchError && (
                <div className="p-4 sm:p-5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/40 text-red-800 dark:text-red-300 text-xs sm:text-sm flex items-center gap-3">
              ...
              <div className="space-y-3.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs sm:text-sm font-bold text-[#717684] dark:text-[#8E95A5] uppercase">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  {SAMPLE_PATCHES.map((sample) => (
                    <div 
                      key={sample.patchId}
                      className="p-4 rounded-xl bg-[#F8F8F5] dark:bg-[#1A1C22] border border-[#E5E5E0] dark:border-[#26282E] flex flex-col justify-between gap-3 hover:border-slate-400 dark:hover:border-slate-600 transition"
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          <span className="px-2.5 py-0.5 rounded-md text-xs font-mono font-bold bg-[#111215] text-white dark:bg-white dark:text-[#111215]">
                            v{sample.version}
                          </span>
                          <span className="text-xs text-[#717684] dark:text-[#8E95A5]">
                            {sample.releaseDate}
                        ...
                        <h5 className="font-bold text-sm sm:text-base text-[#111215] dark:text-white mt-2 line-clamp-1">
                          {sample.title}
                        </h5>
                        <p className="text-xs text-[#5E6472] dark:text-[#8E95A5] mt-1 line-clamp-2">
                          {sample.description}
                        </p>
                      </div>

                      <div className="flex items-center justify-between pt-2.5 border-t border-[#E5E5E0] dark:border-[#26282E] gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded-lg bg-[#EFEFEA] hover:bg-[#E4E4DE] dark:bg-[#252830] dark:hover:bg-[#2F323D] text-[#111215] dark:text-white text-xs font-bold transition flex items-center gap-1.5"
                          >
                            <Download className="w-3.5 h-3.5 text-[#E63B00]" />
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-bold transition flex items-center gap-1.5"
                          >
                            <span>Применить</span>
              ...
              {uploadedPatch && (
                <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-[#15171C] border-2 border-[#111215] dark:border-white shadow-xl space-y-4">
                  <div className="flex flex-wrap items-start justify-between gap-3 pb-3.5 border-b border-[#E5E5E0] dark:border-[#26282E]">
                    <div className="flex items-center gap-3.5">
                      <div className="w-11 h-11 rounded-xl bg-[#111215] dark:bg-white text-white dark:text-[#111215] flex items-center justify-center font-bold">
                        <FileCheck className="w-6 h-6 text-[#E63B00]" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs sm:text-sm font-mono font-bold px-2.5 py-0.5 rounded-md bg-[#F0F0EB] dark:bg-[#252830]">
                            v{uploadedPatch.version}
                          </span>
                          <span className="text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5]">
                            Автор: <strong className="text-[#111215] dark:text-white">{uploadedPatch.author}</strong>
                          </span>
                        </div>
                        <h3 className="text-base sm:text-lg font-black text-[#111215] dark:text-white mt-1">
                          {uploadedPatch.title}
                    ...
                    <button
                      ...
                      className="px-6 py-3 rounded-xl bg-[#111215] hover:bg-[#272A33] dark:bg-white dark:hover:bg-slate-100 text-white dark:text-[#111215] text-xs sm:text-sm font-black flex items-center gap-2 shadow transition disabled:opacity-50"
                    >
                  ...
                  <p className="text-xs sm:text-sm text-[#5E6472] dark:text-[#8E95A5] leading-relaxed">
                    {uploadedPatch.description}
                  </p>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
                    <div className="p-4 rounded-xl bg-[#F8F8F5] dark:bg-[#1A1C22] border border-[#E5E5E0] dark:border-[#26282E]">
                      <span className="font-bold text-[#717684] dark:text-[#8E95A5] block uppercase mb-2 text-xs">
                      ...
                      <ul className="space-y-1.5 font-mono">
                        {uploadedPatch.payload.recipesToAddOrUpdate?.map((r) => (
                          <li key={r.code} className="flex justify-between items-center text-slate-800 dark:text-slate-200">
                            ...
                            <span className="text-xs text-slate-500">{r.components.length} комп. / {r.targetTotalWeightKg} кг</span>
                          </li>
                        ))}
                      </ul>
                    </div>

                    <div className="p-4 rounded-xl bg-[#F8F8F5] dark:bg-[#1A1C22] border border-[#E5E5E0] dark:border-[#26282E] flex flex-col justify-between">
                      <div>
                        <span className="font-bold text-[#717684] dark:text-[#8E95A5] block uppercase mb-2 text-xs">
                          Контрольная сумма пакета:
                        </span>
                        <div className="font-mono text-xs text-slate-600 dark:text-slate-400 break-all leading-relaxed">
                          Checksum: {uploadedPatch.checksum || calculateChecksum(uploadedPatch.payload)}
                        </div>
                      </div>
                      <div className="text-xs sm:text-sm text-emerald-600 dark:text-emerald-400 font-semibold mt-3 flex items-center gap-2">
        ...
        {/* Footer */}
        <div className="p-4 sm:p-5 border-t border-[#E5E5E0] dark:border-[#26282E] bg-[#FBFBFA] dark:bg-[#181A20] flex items-center justify-between text-xs sm:text-sm text-[#717684] dark:text-[#8E95A5]">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-[#EFEFEA] hover:bg-[#E4E4DE] dark:bg-[#252830] dark:hover:bg-[#2F323D] text-[#111215] dark:text-white font-bold transition"
          >
            Закрыть
          </button>
        </div>
